...
fn lower_node(node: &CstNode) -> Stmt {
    match node {
        ...
        CstNode::Expr(e) => lower_expr_stmt(&e.tokens),
        _ => Stmt::Unsupported,
    }
}

fn lower_expr_stmt(tokens: &[Token]) -> Stmt {
    if matches!(
        tokens.first().map(|t| &t.kind),
        Some(TokenKind::Keyword(Keyword::Let))
    ) {
        if let (Some(TokenKind::Identifier(name)), Some(eq_idx)) = (
            tokens.get(1).map(|t| &t.kind),
            tokens.iter().position(|t| t.kind == TokenKind::Eq),
        ) {
            if let Some(expr) = lower_expr_tokens(&tokens[eq_idx + 1..]) {
                return Stmt::Let(name.clone(), expr);
            }
        }
    }
    if let Some(expr) = lower_expr_tokens(tokens) {
